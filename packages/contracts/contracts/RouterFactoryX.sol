...
contract RouterFactoryX is IRouterFactory, Ownable {
  /**
  ...
  function createRouter(address routerSigner, address recipient) external override returns (address) {
    ...
    RouterX(router).init(address(transactionManager), chainId, routerSigner, recipient, msg.sender);
  ...
  function getBytecode() internal view returns (bytes memory) {
    bytes memory bytecode = type(RouterX).creationCode;
