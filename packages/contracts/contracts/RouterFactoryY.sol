...
contract RouterFactoryY is IRouterFactory, Ownable {
  /**
  ...
  function createRouter(address routerSigner, address recipient) external override returns (address) {
    ...
    RouterY(router).init(address(transactionManager), chainId, routerSigner, recipient, msg.sender);
  ...
  function getBytecode() internal view returns (bytes memory) {
    bytes memory bytecode = type(RouterY).creationCode;
